...
import {
  mantenimientosTallerApi,
  vehiculosTallerApi,
  serviciosTallerApi,
  trabajadoresTallerApi,
} from "@/lib/mantenimientos-admin-taller-api"
...
import { MantenimientoTallerFormModal } from "@/components/admin-taller/forms/mantenimiento-form-modal"
import { MantenimientosTallerDashboard } from "@/components/admin-taller/mantenimientos/mantenimientos-dashboard"
import { AdvancedFilters } from "@/components/admin/advanced-filters"
import { DetailsModal } from "@/components/admin/details-modal"

export default function MantenimientosTallerPage() {
  ...
  const [currentFilters, setCurrentFilters] = useState<MantenimientoTallerFilterParams>({})
  ...
  const loadAuxiliaryData = async () => {
    try {
      const [vehiculosResponse, serviciosResponse, trabajadoresResponse] = await Promise.all([
        vehiculosTallerApi.filter("", "ACTIVO"),
        serviciosTallerApi.getAll(),
        trabajadoresTallerApi.getAll(),
      ])
      setVehiculos(vehiculosResponse.content || [])
  ...
  const loadMantenimientos = async (page = 0, size = 10, filters: MantenimientoTallerFilterParams = {}) => {
    try {
      setIsLoading(true)
  ...
  const handleDelete = async (id: number) => {
    if (confirm("¿Estás seguro de que deseas eliminar este mantenimiento?")) {
      try {
        await mantenimientosTallerApi.delete(id)
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Trabajador",
      render: (mantenimiento: MantenimientoResponse) => (
        <span className="text-sm">{mantenimiento.trabajador?.usuario.nombreCompleto || "Sin asignar"}</span>
      ),
    },
    {
      key: "estado",
  ...
  const handleApplyFilters = (filters: MantenimientoTallerFilterParams) => {
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">Panel de mantenimientos para tu taller</p>
        </div>
        ...
        <Tabs defaultValue="dashboard" className="space-y-4">
          ...
          <TabsContent value="dashboard" className="space-y-4">
            <MantenimientosTallerDashboard onRefresh={handleRefresh} />
          </TabsContent>
