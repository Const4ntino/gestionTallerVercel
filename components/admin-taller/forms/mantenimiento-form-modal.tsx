...
interface MantenimientoTallerFormModalProps {
  open: boolean
...
export function MantenimientoTallerFormModal({
  open,
  onOpenChange,
  mantenimiento,
  onSuccess,
}: MantenimientoTallerFormModalProps) {
  ...
  const loadProductos = async () => {
    try {
      ...
      const productosData = await productosTallerApi.getAll()
  ...
  const searchVehiculos = async (search: string) => {
    try {
      const response = await vehiculosTallerApi.filter(search, "ACTIVO")
  ...
  const onSubmit = async (data: MantenimientoFormData) => {
    try {
      ...
      if (mantenimiento) {
        await mantenimientosTallerApi.update(mantenimiento.id, payload)
        toast.success("Mantenimiento actualizado correctamente")
      } else {
        await mantenimientosTallerApi.create(payload)
              ...
              <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-3 p-4 border rounded-lg">
                <div className="space-y-1">
                  ...
                  >
                    <SelectTrigger className="h-9">
                      {loadingProductos ? (
                        ...
                        <SelectValue
                          placeholder={!selectedServicioId ? "Selecciona un servicio primero" : "Selecciona producto"}
                        />
                      )}
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  {producto.productoId > 0 && (
                    <div className="text-xs text-muted-foreground">Precio: ${producto.precioEnUso.toFixed(2)}</div>
                  )}
                </div>
